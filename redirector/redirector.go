...
func (rrs *RedirectionRules) unmarshalInner(rules interface{}) error {
	...
	case map[string]interface{}:
		...
		for match, substitution := range rules {
			if _to, ok := substitution.(string); !ok {
...
func (rd *ReceptionDescriptor) Format(recipient string) string {
...
func (rtr *Redirector) retouch(
	w io.Writer,
	m Mail,
	receptionDescriptor *ReceptionDescriptor,
	...
	err := rfc5322.Scan(
		...
		rfc5322.ScannerHandlerFromFunctions(
			...
			func(br bufio.BufferedReader) error {
				...
				{
					chunks := [][]byte{[]byte(receptionDescriptor.Format(m.Recipient))}
...
func (rtr *Redirector) TryRedirect(m Mail, rd *ReceptionDescriptor) (bool, Mail, string, error) {
	...
	for i, rule := range rtr.Rules {
		...
		if rcpt != m.Recipient {
			...
			m.data = b.Bytes()
			m.Recipient = rcpt
			return true, m, string(newAddr.GetDomain()), nil
		}
	}
	return false, m, "", err
}
